...
SampleTrimmer.slice_from_beginning(
    audio=audio, timedelta=timedelta(seconds=15, milliseconds=100)
)
...
SampleTrimmer.slice_from_to(
    audio=audio,
    start_time=timedelta(seconds=15, milliseconds=100),
    end_time=timedelta(seconds=30, milliseconds=100),
)
