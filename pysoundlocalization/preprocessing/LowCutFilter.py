...
class LowCutFilter(IFrequencyFilter):
    ...
    def apply(self, audio: Audio):
        ...
        """
        print(
            f"Applying LowCutFilter with cutoff frequency {self.__cutoff_frequency} Hz and order {self.__order}"
        )
        ...
        audio.set_audio_signal(lfilter(b, a, audio.get_audio_signal_by_index(index=0)))

    def get_cutoff_frequency(self) -> float:
        """
        Get the cutoff frequency of the low-cut filter.

        Returns:
            float: The cutoff frequency of the low-cut filter in Hz.
        """
        return self.__cutoff_frequency

    def set_cutoff_frequency(self, cutoff_frequency: float) -> None:
        """
        Set the cutoff frequency of the low-cut filter.

        Args:
            cutoff_frequency (float): The cutoff frequency of the low-cut filter in Hz.
        """
        self.__cutoff_frequency = cutoff_frequency

    def get_order(self) -> int:
        """
        Get the order of the low-cut filter.

        Returns:
            int: The order of the low-cut filter.
        """
        return self.__order

    def set_order(self, order: int) -> None:
        """
        Set the order of the low-cut filter.

        Args:
            order (int): The order of the low-cut filter.
        """
        self.__order = order
