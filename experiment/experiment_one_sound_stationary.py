    ...
    source_positions = [
        {
            ...
            int(lowest_sample_rate * 6): RANDOM_SOURCE_B1,
        },
    ]
...
def main():
    ...
    for i in range(100):
        ...
        with open(f"{ts}_experiment_one_sound_stationary.txt", "a") as file:
            file.write(f"Round {i+1}: {result}\n")
            file.flush()  # Flush the content to disk immediately

        print(
            f"Round {i+1}: Appended results to {ts}_experiment_one_sound_stationary.txt."
        )
