...
SampleTrimmer.trim_from_beginning(audio1, timedelta(seconds=30))
SampleTrimmer.trim_from_end(audio1, timedelta(seconds=10))

audio_file2 = "../06_versuch_classroom_syncatmic3_sinus_at_random_points/pi2_audio.wav"
audio2 = Audio(filepath=audio_file2, convert_to_sample_rate=44100)
mic2.set_recording_start_time(datetime(2024, 11, 7, 10, 30, 45, 474498))
print(audio2.get_duration())

SampleTrimmer.trim_from_beginning(audio2, timedelta(seconds=30))
SampleTrimmer.trim_from_end(audio2, timedelta(seconds=10))

audio_file3 = "../06_versuch_classroom_syncatmic3_sinus_at_random_points/pi3_audio.wav"
audio3 = Audio(filepath=audio_file3, convert_to_sample_rate=44100)
mic3.set_recording_start_time(datetime(2024, 11, 7, 10, 30, 46, 550904))
print(audio3.get_duration())

SampleTrimmer.trim_from_beginning(audio3, timedelta(seconds=30))
SampleTrimmer.trim_from_end(audio3, timedelta(seconds=10))

audio_file4 = "../06_versuch_classroom_syncatmic3_sinus_at_random_points/pi4_audio.wav"
...
SampleTrimmer.trim_from_beginning(audio4, timedelta(seconds=30))
SampleTrimmer.trim_from_end(audio4, timedelta(seconds=10))


spectrogram_plot(
    audio_signal=audio1.get_audio_signal(index=0),
...
audio_signal_concatenated = np.concatenate(
    [
        audio1.get_audio_signal(index=0),
        audio2.get_audio_signal(index=0),
        audio3.get_audio_signal(index=0),
        audio4.get_audio_signal(index=0),
    ],
    axis=0,
)

audio_concatinated = Audio.create_from_signal(audio_signal_concatenated, 44100)
...
for i, audio_concatinated_source_signal in enumerate(audio_signal_concatinated_nmf):
    new_audio1_signal = audio_concatinated_source_signal[
        0 : len(audio1.get_audio_signal(index=0))
    ]
    new_audio2_signal = audio_concatinated_source_signal[
        len(audio1.get_audio_signal(index=0)) : len(audio1.get_audio_signal(index=0))
        + len(audio2.get_audio_signal(index=0))
    ]
    new_audio3_signal = audio_concatinated_source_signal[
        len(audio1.get_audio_signal(index=0))
        + len(audio2.get_audio_signal(index=0)) : len(audio1.get_audio_signal(index=0))
        + len(audio2.get_audio_signal(index=0))
        + len(audio3.get_audio_signal(index=0))
    ]
    new_audio4_signal = audio_concatinated_source_signal[
        len(audio1.get_audio_signal(index=0))
        + len(audio2.get_audio_signal(index=0))
        + len(audio3.get_audio_signal(index=0)) : len(audio1.get_audio_signal(index=0))
        + len(audio2.get_audio_signal(index=0))
        + len(audio3.get_audio_signal(index=0))
        + len(audio4.get_audio_signal(index=0))
    ]
